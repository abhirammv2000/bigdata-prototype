> ⚠️ **LEGACY / HISTORICAL DOCUMENT — not the current architecture.**
> This walkthrough describes the earlier stack (local Docker Compose, Streamlit
> frontend, Ollama LLM, Cloud Run + Cloud Composer deployment). The project now
> runs on **GKE with a React frontend and Google Gemini** — see the root
> [`README.md`](../../README.md) for the current setup. Kept here only for
> background on how the pipelines and RAG engine were originally designed.
